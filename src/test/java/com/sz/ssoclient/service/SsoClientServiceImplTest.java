 ...
 * 使用 Mockito mock 登录适配器，不依赖 Spring 上下文或 Sa-Token 真实运行环境。
 * </p>
 */
...
class SsoClientServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        SsoLoginOrchestrator<TestUser> orchestrator = new SsoLoginOrchestrator<>(List.of(
                new PrepareLoginParameterStep<>(),
                new BuildLoginUserStep<>(loginAdapter),
                new CreateLoginResultStep<>(loginAdapter)
        ));
        service = new SsoClientServiceImpl<>(orchestrator);
    }
